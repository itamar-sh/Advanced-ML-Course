...
DEPLOY_PACKAGE_DIR = BASE_DIRECTORY = abspath(join(abspath(__file__), ".."))
BYTES_IN_MB = 1024 ** 2
...
def main() -> None:
    ...
    try:
        deploy_cloudformation_template(
            now_ms=now_ms,
            stack_name="ml-code-bucket",
            template_file_path="./code_bucket.yaml",
        )
        ...
        deploy_cloudformation_template(
            now_ms=now_ms,
            stack_name="create_resources",
            template_file_path="./sam_template.yaml",
        )
